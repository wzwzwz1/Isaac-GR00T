...
def _flash_attention_forward(
    ...
    **kwargs,
    ...
    if flash_attn_varlen_func is None:
        # NPU fallback: manual SDPA (F.scaled_dot_product_attention fails on 310P3)
        scale = 1.0 / (query_states.shape[-1] ** 0.5)
        scores = torch.matmul(query_states, key_states.transpose(-2, -1)) * scale
        if attention_mask is not None:
            scores = scores + attention_mask
        attn_weights = torch.softmax(scores, dim=-1).to(query_states.dtype)
...
class Rope2DPosEmb(nn.Module):
    ...
    def _precompute_freqs_cis(self, device: torch.device) -> torch.Tensor:
        ...
        y_cos, y_sin = torch.cos(y_freqs), torch.sin(y_freqs)  # M, C/4
...
def _apply_rope_input_validation(x, freqs_cis):
    ...
    assert x.shape[-1] == 2 * freqs_cis.shape[-2], (x.shape, freqs_cis.shape)  # NPU: no complex
...
def apply_rope(
    ...
    # NPU: real-valued RoPE (complex not supported on 310P3)
    cos = freqs_cis[..., 0]  # ..., head_dim/4
    sin = freqs_cis[..., 1]
    cos = cos.unsqueeze(-2)  # ..., 1, head_dim/4
    sin = sin.unsqueeze(-2)
